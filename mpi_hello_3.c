...
int main(int argc, char *argv[]) {
    ...
    if (argc != 2) {
        if (rank == 0) {
            printf("Uso: mpiexec -n <número_de_procesos> ./mpi_hello_3 <número_de_procesos>\n");
        }
        MPI_Finalize();
        return 1;
    }
    ...
    // Buffer para recibir mensajes de respuesta
    char response[100];

    // Proceso 0 recibe mensajes de todos los demás procesos y envía respuestas
    if (rank == 0) {
        for (int source = 1; source < size; source++) {
            ...
            // Enviar respuesta de vuelta al proceso que envió el mensaje
            snprintf(response, sizeof(response), "Hello from process 0 to process %d!", source);
            MPI_Send(response, sizeof(response), MPI_CHAR, source, 0, MPI_COMM_WORLD);
        }
    } else {
        ...
        // Recibir respuesta del proceso 0
        MPI_Recv(response, sizeof(response), MPI_CHAR, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        printf("Proceso %d recibió respuesta de proceso 0: %s\n", rank, response);
    }
